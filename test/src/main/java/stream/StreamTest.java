...
//	min(comparator)
	...
	/**
	 * groupingBy trong stream API (gom nhóm)
	 */
	public static void group() {
		...
		var newLst = lst.stream()
			.collect(Collectors.groupingBy(m -> m.getGener(), Collectors.toList()));
		System.err.println(newLst);
		/**
		 * {MENTOR=[ {title: c ,like:40, gener: MENTOR}], ACTION=[ {title: a ,like:10, gener: ACTION},  {title: d ,like:20, gener: ACTION}], THREAF=[ {title: b ,like:30, gener: THREAF}]}
		 */
		var newLst2 = lst.stream()
				.collect(Collectors.groupingBy(Film::getGener, Collectors.mapping(Film::getTitle, Collectors.joining(",")))); // kết hợp mapping lấy ra giá trị cần lấy và cách nhau bởi dấy phẩy
		System.err.println(newLst2);
		/**
		 * {MENTOR=c, ACTION=a,d, THREAF=b}
		 */
		
	}
	
	/**
	 * partitioningBy ( chia thành từng phần)
	 */
	public static void partitioningBy() {
		...
		/**
		 * kết quả trả về sẽ chia thành 2 phần theo điều kiện ( 1. là lớn hơn 20, 2. là nhỏ hơn 20)
		 * partitioningBy có 2 cách truyền tham số
		 * Cách 1: là truyền mỗi điều kiện
		 * Cách 2: truyền thêm Collector<?,?> downstream (downstream: thực hiện tiếp biến đổi trên kết quả))
		 */
		// cách 1
		 var newLst = lst.stream().collect(Collectors.partitioningBy(m -> m.getLike() > 20));
		 System.err.println(newLst);
		 /**
		  * {false=[ {title: a ,like:10,  {title: d ,like:20], true=[ {title: b ,like:30,  {title: c ,like:40]}
		  */
		 // cách 2
		 var newLst2 = lst.stream().collect(
				 Collectors.partitioningBy(m -> m.getLike() > 20,
